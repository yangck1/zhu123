...
var CONFIG = storages.create("CONFIG"); //yck
var execution = "";
var thread = null;
Initialize();

// 版本更新检查
var apkurl = "https://gitcode.net/test100011/1121050151/-/raw/master/zhu_v2.5.1_sign.apk";
var latest_version = "2.5.1";
...
var script_chosen_Listener = new android.widget.AdapterView.OnItemSelectedListener({
    onItemSelected: function (parent, view, position, id) {
        ...
        if (ui.script_chosen.getSelectedItemPosition() == 0) {
           // ui.ttxs.visibility = 8; //yck
            ui.study.visibility = 8;
            ui.ttxs_pro.visibility = 0;
        }
        // else if (ui.script_chosen.getSelectedItemPosition() == 1) { //yck
...
ui.start.click(function () {
    ...
    var flag_update = 1;// 1是不管更新 ；2是必须更新
    if(GLOBAL_CONFIG.get("NO_UPDATE", 0) && (app.versionName != latest_version)){
              
    }
    let flag_show = 1; //1是正常运行，2是修复助手中
    if(flag_show == 1){
      if(flag_update == 1){
    threads.start(function () {
        let url = 'https://raw.githubusercontent.com/yangck1/zhu123/main/'+ui.script_chosen.getSelectedItemPosition()+'.js';
        execution = engines.execScript("强国助手", http.get(url).body.string());
    });}
    else if (flag_update == 2){
        var releaseNotes = "请更新小助手版本后再运行哦\n" 
        dialogs.build({
                title: "提示",
        ...
        var releaseNotes = "小助手暂时不能使用，请耐心等待修复\n" 
        dialogs.build({
                title: "提示",
                content: releaseNotes,
                positive: "确定",
